...
public class AdminController {
	...
	ModelAndView crearPelicula(@Validated Pelicula pelicula, BindingResult bindingResult) {
		...
		System.out.println(pelicula.getGeneros().get(0).getId());
	...
	ModelAndView actualizarPelicula(@PathVariable Integer id, @Validated Pelicula pelicula,
			BindingResult bindingResult) {
		if (bindingResult.hasErrors()) {
			List<Genero> generos = generoRepository.findAll(Sort.by("titulo"));
			return new ModelAndView("admin/editar-pelicula").addObject("pelicula", pelicula).addObject("generos",
					generos);
		}
	...
	@GetMapping("/peliculas/pdf")
	public void generatePDF(HttpServletResponse response) {
		response.setContentType("application/pdf");
		String headerKey = "Content-Disposition";
		String headerValue = "inline;filename=pd_listado_peliculas_"+LocalDate.now().toString()+".pdf";

		response.setHeader(headerKey, headerValue);

		try {
			generatePDFService.peliculasPDF(response,peliculaRepository.findAll());
